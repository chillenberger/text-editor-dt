...
function createFileInPathTree(file: File, pathTree: PathTree) {
  ...
  while ( splitPath.length >= 0 && !!subPath ) {
    ...
    if ( nextNode.children === null ) throw new Error(`Path ${subPath} not found, ${subPath} is a file`);
...
export{ findDir, createFileInPathTree, deleteFileFromPathTree, getContentTypeFromPath, alphabetizeDir };