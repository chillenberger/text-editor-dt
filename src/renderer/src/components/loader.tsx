...
export function LoadingAnimation({ size = 'md' }: LoadingAnimationProps) {
  ...
  return (
    <div className={`flex items-center justify-center ${containerClasses[size]}`}>
      <span
        className={`${sizeClasses[size]} bg-ide-accent rounded-full animate-bounce`}
        style={{ animationDelay: '0ms' }}
      />
      <span
        className={`${sizeClasses[size]} bg-ide-accent/80 rounded-full animate-bounce`}
        style={{ animationDelay: '150ms' }}
      />
      <span
        className={`${sizeClasses[size]} bg-ide-accent/60 rounded-full animate-bounce`}
