...
export default function useChat(projectDirectory: string, folders: string[] | null) {
  ...
  useEffect(() => {
    console.log("Initializing agent in useChat with projectDirectory:", projectDirectory, "folders:", folders, "agentId:", agentId);
    if( folders ) window.electron.ipcRenderer.invoke('initialize-agent', projectDirectory, folders, agentId);
  ...
  const chatRequestStream = useCallback(async (userQuery: string, projectName: string, folders: string[]) => {
    ...
    try {
      window.electron.ipcRenderer.send('chat-stream', userQuery, responseId, folders, sessionValue, timeLastRequest);
    } catch (error) {
