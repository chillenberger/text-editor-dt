...
function useManageFiles(folder: string | null): ManagedFileSystem {
  ...
  async function getFile(path: string): Promise<DirEditRsp> {
    ...
    return {
      ...
      file: content !== null ? { path, content } : undefined
    }
  }
...
function useManageActiveFile(dir: ManagedFileSystem[]) {
  ...
  useEffect(() => {
    activeDir.current = dir.find((mfs) => mfs.dir.title === activeFile?.path.split('/')[0]) || null
  }, [dir, activeFile])
...
function useVirtualDirectory(
  projectName: string,
  dirs: ManagedFileSystem[]
): VirtualManagedFileSystem {
  const managedFileSystems = dirs

  const virtualDir: PathTree = {
    title: projectName,
    pathType: 'dir',
    children: dirs.map((mfs) => mfs.dir)
  }
  ...
  async function _useManageFilesWrapper(
    command: string,
    filePath: string,
    content?: string
  ): Promise<DirEditRsp> {
