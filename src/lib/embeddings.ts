...
export async function chunkDocument(text: string): Promise<Chunk[]> {
  const chunker = await RecursiveChunker.create({
    // TODO: change this to a semantic chunker
    chunkSize: 512
  })
...
export async function updateEmbeddingsForFile(
  filePath: string,
  content: string,
  localStorage: any
): Promise<void> {
  ...
  const existingHashMap: Map<
    string,
    { id: number; embedding_vector: Buffer; chunk_index: number }
  > = new Map(
    existingEmbeddings.map((e: any) => [
      e.content_hash,
      { id: e.id, embedding_vector: e.embedding_vector, chunk_index: e.chunk_index ?? 0 }
    ])
  )
...
export async function searchEmbeddings(
  ...
  topK: number = 5,
  filePaths: string[]
): Promise<Array<{ file_path: string; score: number }>> {
...
export async function searchEmbeddingsDistinct(
  query: string,
  localStorage: any,
  topK: number = 5,
  filePaths: string[]
